  ...
  test("create token and find by token") {
    transactorResource.use { xa =>
      ...
      for
        ...
        _     <- repo.create(userId, token, expiresAt, Some("test-agent"), Some("127.0.0.1"), None, None).transact(xa)
  ...
  test("create token with null device info and ip address") {
    transactorResource.use { xa =>
      ...
      for
        ...
        token = java.util.UUID.randomUUID.toString.replace("-", "")
        expiresAt = Instant.now.plusSeconds(604800)
        _     <- repo.create(userId, token, expiresAt, None, None, None, None).transact(xa)
        found <- repo.findByToken(token).transact(xa)
      yield
        assert(found.isDefined)
  ...
  test("revokeAllByUserId revokes all active tokens for a user marking them as theft") {
    transactorResource.use { xa =>
      ...
      for
        ...
        expiresAt = Instant.now.plusSeconds(604800)
        _ <- repo.create(userId, tok1, expiresAt, None, None, None, None).transact(xa)
        _ <- repo.create(userId, tok2, expiresAt, None, None, None, None).transact(xa)
  ...
  test("revokeAllByUserId does not affect other users") {
    transactorResource.use { xa =>
      ...
      for
        ...
        tok1 = java.util.UUID.randomUUID.toString.replace("-", "")
        expiresAt = Instant.now.plusSeconds(604800)
        _ <- repo.create(user1, tok1, expiresAt, None, None, None, None).transact(xa)
